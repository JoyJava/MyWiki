...
-使用等价转化规则
    MySQL 可以使用一些等价变换来简化并规范表达式, 可以合并和减少比较, 还可以移除一些恒定成立和一些恒定不成立的判断, 如:(5=5 and a>5 ) 将会改写成 a>5,再例如: (a<b and b=c) and a=5 会改成为 a5 and b=c and a=i>5
- 优化 COUNT, MIN, MAX
...
- High_priority Low_priority
    high_priority: 用于 SELECT 语句的时候, MySql 会将此 SELECT 语句重新调度到所有正在等待表锁以便修改数据的语句之前, 实际上 MySQL 讲其房子表的队列的最前面,而不是按照常规顺序等待. 还可以用于 INSERT 语句, 其效果只是简单抵消了全局 Low_priority 设置对该语句的影响
    low_priority: 会让该语句一直处于等待状态, 只要队列中还有需要访问同一个表的语句--及时是那些比该语句还晚提交到服务器的语句
- delayed
    作用于 INSERT 和 REPLACE 有效, MySql 将使用该提示语句立即返回给客户端, 并将插入的行数据放入缓冲区, 然后在表空闲时批量将数据写入. 如: 日志系统. 该提示会导致函数 last_insert_id 无法工作
- straight_join
    可以防止在 select 语句的 select 关键字之后, 也可以放置在任何两个关联表的名字之间, 第一个用法是让查询中所有的表按照在语句中出现的顺序进行关联, 第二个是固定其前后两个表的关联顺序
- sql_small_result 和 sql_big_result
    对 select 有效,它告诉优化器对 group by 和 distinct 查询如何使用零时表及排序
- sql_buffer_result
    提示优化器将查询结果放入到一个临时表, 然后竟可能块的释放表锁.
- sql_cache 和 sql_no_cache
    提示 MySQL 个结果集是否应该缓存在查询缓存中
- sql_calc_found_rows
    这并不是一个优化器提示. 它不会告诉优化器任何关于执行计划的东西.它会让 MySql 返回结果集包含更多信息,
    查询中加上该提示 MySQL 计算除去 limit 子句后这个查询要返回的结果集的总数据, 而实际上返回 limit 要求的结果集, 可以通过 found_row 获得这个值
- for update 和 lock in share mode
    主要控死 select 语句的锁机制
- use index 和 ignore index 和 force index
    这个告诉优化器使用或者不实用那些索引来查询记录
    use index 和 force index 基本相同, 除了一点: force index 告诉优化器全表扫描的成本会远远高于索引扫描, 当发现优化器选择了错误的索引.

#### MySQL 5.0和 更新版本中, 新增了一些参数用来控制优化器的行为:
- optimizer_search_depth
    控制优化器在穷举执行计划时的限度, 如果查询长时间处于 `"Statistics"` 状态, 那么可以考虑调低该参数
- optimize_prune_level
    默认打开的, 这个让优化器会根据需要扫描行数来决定是否跳过某些执行计划
- optimize_switch
    5.1 中通过这个参数来控制禁用所有合并的特征

## MySQL配置工作原理
    长期使用的设置应该写在全局配置文件, 而不是在命令行特别指定. 如debian 下: /etc/mysql/my.cnf才是 mysql读取的配置文件而不是/ etc/my.conf
    确定当前配置文件: which mysqld
    mysqld --verbose --help | grep -A 1 'Default options'

### 语法, 作用域和动态性
- query_cache_size 变量是全局的
- sort_buffer_size 变量默认是全局相同的, 但是Thread 可以单独设置
- join_buffer_size 变量也有全局默认值并且每个线程可以单独设置

### 常用的变量和动态修改效果
- key_buffer_size 设置变量可以一次性缓冲区分配所有指定的空间
- Thread_cache_size 可以保持在 cache 的线程数. 线程缓存:
- TableCache
